...
class ThreeViewController: UIViewController {
    ...
    @IBAction func level3ButtonTapped(_ sender: Any) {
        sharedController.addRiddleAlert(riddle: "Some with fragrance, some with colors, some with medicine. All bring a smile to her face.", answer: "flowers", clue: "Hold for Three", viewController: self, button: nil, gesture: topRightHold, view: topRightHoldView, segue: "openingSegue")
    }
    ...
    @IBAction func topRightHoldComplete(_ sender: Any) {
        if topRightHold.state == .ended {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5, execute: {
                self.sharedController.addRiddleAlert(riddle: "The French speak this language so well, they named Paris after me.", answer: "romance", clue: "Hold for Three", viewController: self, button: nil, gesture: self.bottomLeftHold, view: self.bottomLeftHoldView, segue: "openingSegue")
            })
        }
    }
    ...
    @IBAction func bottomLeftHoldComplete(_ sender: Any) {
        if bottomLeftHold.state == .ended {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5, execute: {
                self.sharedController.addRiddleAlert(riddle: "Listen to me, and you'll never go wrong.", answer: "heart", clue: "Hold for Three", viewController: self, button: nil, gesture: self.bottomMiddleHold, view: self.bottomMiddleHoldView, segue: "openingSegue")
            })
        }
    }
    ...
    @IBAction func bottomRightHoldComplete(_ sender: Any) {
        if bottomRightHold.state == .ended {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.5, execute: {
                self.sharedController.addRiddleAlert(riddle: "The key to a shallow woman's heart.", answer: "diamonds", clue: "Hold for Three", viewController: self, button: nil, gesture: self.topLeftHold, view: self.topLeftHoldView, segue: "openingSegue")
            })
        }
    }
